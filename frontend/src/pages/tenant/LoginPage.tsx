...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user?.role === 'tenant_user' && user.tenant_slug === tenant) {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      await login(data.username, data.password, tenant);
      navigate(`/${tenant}/dashboard`);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-950 via-primary-900 to-primary-800 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Kohlplan</h1>
          <p className="text-primary-300 mt-1 text-sm font-mono">{tenant}</p>
        </div>
